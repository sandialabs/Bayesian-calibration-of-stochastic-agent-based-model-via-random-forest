...
surr_loc = env + "/results/bayesian_calibration_random_forest/surrogate/surrogate.pkl"
...
# ------------------------------------------------------------------------------
hscores_crps = scoring.crps_ensemble(data_real.hospitalizations.dropna().to_numpy(), intervals[0]['prediction'].T)
dscores_crps = scoring.crps_ensemble(data_real.deaths.dropna().to_numpy(), intervals[1]['prediction'].T)

data_mask = ~np.isnan(intervals[0]['prediction'])
hmean = intervals[0]['prediction'][data_mask].mean(axis=0)
dmean = intervals[1]['prediction'][data_mask].mean(axis=0)
hstd = intervals[0]['prediction'][data_mask].std(axis=0)
dstd = intervals[1]['prediction'][data_mask].std(axis=0)
hscores_dss = ((data_real.hospitalizations.dropna().to_numpy() - hmean) / hstd)**2 + 2*np.log(hstd)
dscores_dss = ((data_real.deaths.dropna().to_numpy() - dmean) / dstd)**2 + 2*np.log(dstd)
...
# Add CRPS and DSS
xplot = np.arange(len(hscores_crps))
sns.lineplot(x=xplot, y=hscores_crps, ax=axes[1,0], color=colors[3], lw=thick, label="CRPS")
axes[1,0].yaxis.label.set_visible(False)
sns.lineplot(x=xplot, y=hscores_dss, ax=axes[2,0], color=colors[7], lw=thick, label="DSS")
axes[2,0].yaxis.label.set_visible(False)
axes[2,0].xaxis.label.set_visible(False)

sns.lineplot(x=xplot, y=dscores_crps, ax=axes[1,1], color=colors[3], lw=thick)
axes[1,1].yaxis.label.set_visible(False)
sns.lineplot(x=xplot, y=dscores_dss, ax=axes[2,1], color=colors[7], lw=thick)
